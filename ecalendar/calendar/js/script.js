...
function getEvents(e, data, td_id) {
	...
	if ($("#admin").val() == "true" || $('#member').val() == "true") {
...
if (data[i].heure_sortie == "0000-00-00 00:00:00") 
var sortie = ""; 

else {

				timesortie = data[i].heure_sortie.replace(/-|:/g, " ").split(" ").slice(3, 6);
				
	
				timesortie = new Date(2000, 01, 01, timesortie[0], timesortie[1], timesortie[2]);

				timesortie = {
					hours: timesortie.getHours(),
					minutes: timesortie.getMinutes() < 10 ? "0" + timesortie.getMinutes() : timesortie.getMinutes(),
				
				}

				if (time_format == "standard") {
					timesortie.hours = (timesortie.hours > 12) ? timesortie.hours - 12 : timesortie.hours;

					timesortie = timesortie.hours + ":" + timesortie.minutes;
				}
				else if (time_format == "military") {
					timesortie = timesortie.hours + ":" + timesortie.minutes;
				}
	var sortie = " à " + timesortie; 			
}
				if (data[i].location === null) {
					$(".modal-body").append("<div id='event'><span class='title'><h4 style='margin: 24px 0px'>" + data[i].title + "</h4></span><span class='time'><span style='width: 150px; float: left;'><span class='glyphicon glyphicon-time' style='top: 2px'></span> de " + time + sortie +  "</span></span></div>");
				}
				else {
					$(".modal-body").append("<div id='event'><span class='title'><h4>" + data[i].title + "</h4><p>at <i>" + data[i].location + "</i></p></span><span class='time'><span style='width: 150px; float: left;'><span class='glyphicon glyphicon-time' style='top: 2px'></span> de " + time + sortie +  "</span></span></div>");
				}
			}
			catch (err) {}
		}
		...
		$(".modal-body").append("<span class='not-found'>Pas d'évènements pour l'instant.</span>");
	}
}
...
function addEvent(date) {
	...
	if (time_format == "standard") {
		$("#inputTime, #inputTime2").timepicker({
			timeFormat: 'hh:mm tt'
		});
	}
	else {
...
timesortie = form.timesortie.split(" ")[0];
				original = timesortie.split(":");
				
				if (parseInt(original[0]) == 12) {
					timesortie = "00:" + original[1];
				}
			}
			else {
				time = form.time.split(" ")[0];
				...
				timesortie = form.timesortie.split(" ")[0];
				originalsortie = timesortie.split(":");
				

				if (parseInt(original[0]) != 12) {
					timesortiePm = parseInt(originalsortie[0]) + 12;
					timesortie = timesortiePm + ":" + originalsortie[1];
				}
			}
		}

		timestamp = date + " " + time;
		timestampsortie = date + " " + timesortie;

		if ((title && loc && timestamp) !== "") {
			form_data = { 1: title, 2: loc, 3: timestamp, 4: timestampsortie };
...
var modalAddEvent = { header: "<button type='button' class='close' data-dismiss='modal' aria-hidden='true'>&times;</button><h3 id='myModalLabel'>Ajouter un évènement </h3>",
					 	...
					 	"<div class='form-group'>" +
										"<label class='col-sm-3 control-label' for='inputLocation'>Nom du projet</label>" +
										"<div class='col-sm-7'>" +
											"<input type='text' id='inputLocation' class='form-control' maxlength='26' placeholder='Nom du projet' />" +
										"</div>" +
									"</div>" +
									"<div class='form-group'>" +
									...
									"</div>" +
									"<div class='form-group'>" +
										"<label class='col-sm-3 control-label' for='inputTime'>Heure d'entrée</label>" +
										"<div class='col-sm-7'>" +
											"<input type='text' id='inputTime' class='form-control' maxlength='5' placeholder='Heure' />" +
										"</div>" +
					 					"<div class='form-group'>" +
										"<label class='col-sm-3 control-label' style='margin-top:15px;' for='inputTime2'>Heure de sortie</label>" +
										"<div class='col-sm-7'>" +
											"<input style='margin-top:15px;' type='text' id='inputTime2' class='form-control' maxlength='5' placeholder='Heure' />" +
										"</div>" +
									"</div>" +
					 				"<a href='https://calendar.google.com/calendar/render' target='blank_'>Disponibilité</a>"+
								"</form>",
